...
class Tracklist:
	...
	def __init__(self, url=None, soup=None, fetch=True):
		...
		if self.url:
			try:
				...
				self.tracklist_id = "unknown"
		else:
			self.tracklist_id = "local"

		if self.soup:
			self.fetch(soup=self.soup)
		elif fetch and self.url:
			self.fetch()

	def fetch(self, soup=None):
		""" Load Tracklist details from url or soup. """
		if soup:
			self.soup = soup
		...
		if not self.soup:
			raise ValueError("No content to fetch. Provide URL or soup.")
		...
		if left_pane:
			...
			print("Warning: Could not find metadata (div id='left'). Page might be blocked or structure changed.")
			self.date_recorded = None
			self.interaction_details = {}
			self.num_tracks = 0
			self.num_tracks_IDed = 0
			self.genres = []
			self.DJs = []
			self.sources = {}
		...
		if not track_divs:
			print("Warning: No tracks found (div class='tlpItem').")
		for track_div in track_divs:
			t = Track(track_div) # TODO Get external links?
			
			# Check for mashup or linked track (w/)
			is_mashup = track_div.find('i', {'title': 'mashup linked position'})
			is_linked = 'con' in track_div.get('class', [])
			
			# Also check for "w/" in track number if class check fails (though 'con' class seems standard for these)
			if not is_linked:
				track_num_span = track_div.find('span', id=re.compile('_tracknumber_value'))
				if track_num_span and 'w/' in track_num_span.text:
					is_linked = True

			if (is_mashup or is_linked) and self.tracks: 
				self.tracks[-1].add_subsong(t)
			else:
				self.tracks.append(t)

		# Load track numbers (adapted from https://github.com/globalnomad/quickCUE/blob/master/quickCUE.py)
		self.track_nums = [span.text.strip() for span in soup.find_all('span', id=re.compile('_tracknumber_value'))]
		# Cues are now handled in Track class
	...
	def load_metadata(self, left_pane):
		...
		try:
			...
			for person_place in tracklist_DJs_location:
				if re.search(r"/dj/", person_place.get("href")):
					tracklist_DJs.append(person_place.text)
				if re.search(r"/source/", person_place.get("href")):
...
class Track:
	...
	def __init__(self, soup, fetch=False):
		...
		# Get cue time
		try:
			cue_input = soup.find('input', id=re.compile('_cue_seconds'))
			self.cue_seconds = int(cue_input['value']) if cue_input else 0
